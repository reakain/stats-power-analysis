...
# print(traces.shape)
# print(ptext)
# first byte plaintext for all traces
# print(ptext[:,0])
# hws = np.zeros((256, N_traces))
# for k in range(256):
#     c = sbox[np.bitwise_xor(ptext[:,0],k)]
#     hws[k] = c
...
for p in range(255):
    ...
    mean = np.mean(c, axis=0, where= c!=0)
    Es[p] = mean
...
stopt = time.time()
print('Run time is {} seconds'.format(stopt-startt))
print('Key is: ')